...
class StableDiffusionGenerator(ImageGenerator):
    ...
    async def edit_image(self, image: np.ndarray, prompt: str) -> Tuple[np.ndarray, Any]:
        ...
        try:
            ...
            # Create a mask that reveals a small portion of the image
            # This helps maintain continuity while allowing for changes
            mask = Image.new("RGB", pil_image.size, (0, 0, 0))  # Black mask (no changes)
            # Add a white rectangle in the bottom area - this will be inpainted
            # The top part will remain similar to maintain consistency
            width, height = pil_image.size
            bottom_area = (0, height // 3, width, height)
            draw_area = Image.new("RGB", (width, height * 2 // 3), (255, 255, 255))
            mask.paste(draw_area, (0, height // 3))
            
            mask_path = "/tmp/temp_mask_for_sd.png"
            mask.save(mask_path, format="PNG")
            ...
            with open(mask_path, "rb") as mask_file:
                mask_base64 = f"data:application/octet-stream;base64,{base64.b64encode(mask_file.read()).decode("utf-8")}"
